...
userRoute.route('/get-displayname').get(checkToken, (req, res) => {
  console.log('get-user', req.body);
  return res.status(200).send(req.body.user.displayName);
});

userRoute.route('/update-displayname').post(checkToken, (req, res) => {
  console.log('update-user', req.body);
  USER.findByIdAndUpdate(req.body.user._id, { displayName: req.body.displayName })
    .then((user) => {
      if (!user) {
        console.log('Error:: Failed to find user');
        return res.status(500).send('Failed to login');
